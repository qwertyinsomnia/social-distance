...
class yolonet():
    ...
    def get_rectangle_lists(self, boxes, frame):
        # print(len(boxes))
        for box in boxes:
            width = box[2]
            height = box[3]
            # print(width, height)
            left_diff = (height - width) // 2
            x = box[0] - left_diff
            rectangle = frame[box[1]:(box[1] + box[3]), box[0]:(box[0] + box[2])]

            landmarks = self.posedetector.detect(rectangle)
            landmarks = self.get_image_landmarks(landmarks, box)
            print(landmarks)
            # cv2.imshow("winName", rectangle)
            # cv2.waitKey(0)
        return


    def get_image_landmarks(self, landmarks, box):
        landmarks_list = []
        for i in range(13):
            temp = list(landmarks[i])
            temp[0] += box[0]
            temp[1] += box[1]
    ...
    def detect(self, frame):
        ...
        self.get_rectangle_lists(results, frame)
...
if __name__=='__main__':
    ...
    parser.add_argument("--imgpath", type=str, default='1.jpg', help="image path")
    parser.add_argument('--confThreshold', default=0.5, type=float, help='class confidence')
    parser.add_argument('--nmsThreshold', default=0.5, type=float, help='nms iou thresh')
    parser.add_argument('--objThreshold', default=0.5, type=float, help='object confidence')
    args = parser.parse_args()

    net = yolonet('yolov5l.onnx', 0.5, 0.5, 0.5)
    srcimg = cv2.imread('dataset/Wildtrack_dataset/Image_subsets/C1/00000000.png')
